...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let number__containerbutton = document.querySelector(".number__container button")
  let selectedSize = 1;
  ...
  document.querySelectorAll('.ri-zoom-in-line').forEach(item => {
    item.addEventListener('click', (event) => {
      ...
      number__containerbutton.addEventListener("click", () => {
        let jsbasket = JSON.parse(localStorage.getItem("jsbasket")) || [];
        const productExists = jsbasket.some(item => item.modalname === title && item.modalprize === prize);
        if (!productExists) {
          jsbasket.push({ modalname: title, modalprize: prize, modalsnumber: modalnumber.value });
          localStorage.setItem("jsbasket", JSON.stringify(jsbasket));
        }
        console.log(jsbasket);
